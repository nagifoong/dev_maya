...
class LegIk(object):
    def create(self, jnts, biped=True, mirror=False, scale=1):
        # create ik con
        ...
        ik_con_list = shape_gen.create('square', name='{}{}'.format(leg_name, common.TYPE_LIST['ctrl']),
                                       groups=['offset', 'constraint', 'mirror'], color=side, const_pxy=True)
        self.ik_con = ik_con_list[-1]
        # resize
        pm.move(self.ik_con.cv, 0, 0, .5, os=1, r=1)
        pm.scale(self.ik_con.cv, scale, scale, scale, os=1, r=1)

        # make ik con on ground and aim at foot
        # biped: make controller pivot at ankle and snap to ground
        # quad: make controller pivot at ball and snap to ground
        if biped:
        ...
        # mirror main controller
        if mirror:
            ik_con_list[2].sx.set(-1)

        # attributes
        self.ik_con.addAttr('roll', at='double', dv=0, k=1)
        self.ik_con.addAttr('rollHeight', at='double', dv=60, k=1)
        self.ik_con.addAttr('ballYaw', at='double', dv=0, k=1)
        self.ik_con.addAttr('toeTap', at='double', dv=0, k=1)
        self.ik_con.addAttr('toeTipYaw', at='double', dv=0, k=1)
        self.ik_con.addAttr('toeTipPitch', at='double', dv=0, k=1)
        self.ik_con.addAttr('bank', at='double', dv=0, k=1)
        self.ik_con.addAttr('footYaw', at='double', dv=0, k=1)
        self.ik_con.addAttr('heelYaw', at='double', dv=0, k=1)
        self.ik_con.addAttr('heelPitch', at='double', dv=0, k=1)

        # space for ik
        # TODO: might need to move out of function and use pelvis jnt to replace hip jnt to avoid cycle
        ik_con_space_grp = common.create_space_attr(ik_con_list[1], parents=[jnts[0], 'm_root_jnt', 'world'],
                                                    nice_name=['hip', 'root', 'world'], con=self.ik_con)
        ...
        self.pv_con = pv_con_list[-1]
        pos = joints.find_pv(jnts[1:4], multiplier=5)
        pv_con_list[0].t.set(pos)
        self.pv_con.rx.set(-90)
        pm.makeIdentity(self.pv_con, a=1, r=1)

        # mirror pv controller
        if mirror:
            pv_con_list[2].sx.set(-1)

        # resize
        pm.move(self.pv_con.cv, 0, 0, 1, os=1, r=1)
        pm.scale(self.pv_con.cv, scale * .8, scale * .8, scale * .8, os=1, r=1)

        # TODO: might need to move out of function and use pelvis jnt to replace hip jnt to avoid cycle
        pv_con_space_grp = common.create_space_attr(pv_con_list[1],
                                                    parents=[ik_con_list[-2], jnts[0], 'm_root_jnt', 'world'],
                                                    nice_name=['self.ik_con', 'hip', 'root', 'world'], con=self.pv_con)

        # find & create locators
        self.ik_con.addAttr('pivotCtrlVis', at='double', min=0, max=1, dv=0, k=1)
        piv_grp = common.create_name(side='', name=leg_name[:-1], fn='piv', _type='group', create=True, find=True)
        self.ik_con.pivotCtrlVis.connect(piv_grp.v)
        ...
        self.ik_con.addChild(piv_grp)
        self.piv_locs = piv_locs
        pm.matchTransform(piv_grp, self.ik_con)
        piv_grp.s.set([1] * 3)
        ...
        self.ik_con.bank.connect(bank_in_cond.firstTerm)
        self.ik_con.bank.connect(bank_in_cond.colorIfFalseR)
        ...
        bank_out_grp = common.create_name(side='', name='{}_bankOut'.format(leg_name[:-1]), fn='driven', _type='group',
                                          create=True)
        # pivot
        pm.matchTransform(bank_out_grp, self.ik_con)
        ...
        self.ik_con.bank.connect(bank_out_cond.firstTerm)
        self.ik_con.bank.connect(bank_out_cond.colorIfFalseR)
        ...
        pm.matchTransform(foot_grp, self.ik_con)
        piv_locs[-1].t.connect(foot_grp.rotatePivot)
        piv_locs[-1].t.connect(foot_grp.scalePivot)
        piv_locs[-1].t.connect(self.ik_con.rotatePivot)
        piv_locs[-1].t.connect(self.ik_con.scalePivot)

        self.ik_con.footYaw.connect(foot_grp.ry)
        ...
        self.ik_con.roll.connect(roll_neg_cond.firstTerm)
        self.ik_con.roll.connect(roll_neg_cond.colorIfTrueR)
        ...
        heel_rot_sum = common.create_name(side='', name='{}_rot_drn'.format(leg_name[:-1]), _type='plusMinusAverage',
                                          create=True)
        self.ik_con.heelPitch.connect(heel_rot_sum.input3D[0].input3Dx)
        self.ik_con.heelYaw.connect(heel_rot_sum.input3D[0].input3Dy)
        ...
        tt_drn_sum = common.create_name(side='', name='{}_max_roll'.format(leg_name[:-1]), _type='plusMinusAverage',
                                        create=True)
        self.ik_con.toeTipPitch.connect(tt_drn_sum.input3D[0].input3Dx)
        self.ik_con.toeTipYaw.connect(tt_drn_sum.input3D[0].input3Dy)
        ...
        # driven
        self.ik_con.ballYaw.connect(toe_con_list[0].ry)
        self.ik_con.toeTap.connect(toe_con_list[1].rx)

        """ball"""
        ball_con_list = shape_gen.create('cube', name='{}ball_{}'.format(leg_name, common.TYPE_LIST['ctrl']),
                                         groups=['offset', 'auto', 'driven'], color=side)
        self.ball_con = ball_con_list[-1]
        ...
        if biped:
            ...
            pm.delete(pm.aimConstraint(jnts[3], ball_con_list[0], aimVector=[0, 0, -1], upVector=[0, 1, 0]))
            pm.move(ball_con_list[-1].cv, 0, 0, jnts[4].tx.get() / -2.0, os=1, r=1)

        # roll & roll height
        roll_pos_cond = common.create_name(side='', name='{}_pos_roll'.format(leg_name[:-1]), _type='condition',
                                           create=True)
        roll_pos_cond.operation.set(2)
        self.ik_con.roll.connect(roll_pos_cond.firstTerm)
        self.ik_con.roll.connect(roll_pos_cond.colorIfTrueR)
        roll_pos_cond.colorIfFalse.set([0] * 3)

        roll_heig_mult = common.create_name(side='', name='{}_roll_height'.format(leg_name[:-1]),
                                            _type='multiplyDivide',
                                            create=True)
        self.ik_con.rollHeight.connect(roll_heig_mult.i1x)
        ...
        roll_pos_mult.outputX.connect(ball_con_list[-2].rx)
        ...
        self.ik_con.addAttr('stretchyAttr', at='enum', en='____:', k=1)
        self.ik_con.stretchyAttr.set(l=1)
        self.ikh = leg_ik_sys.ikh
        leg_ik_sys.copy_attr(leg_ik_sys.ik_con, self.ik_con)

        pm.parent(leg_ik_sys.pv_con, self.pv_con)
        pm.matchTransform(leg_ik_sys.pv_con, self.pv_con)
        leg_ik_sys.copy_attr(leg_ik_sys.pv_con, self.pv_con)
        ...
        # create spring solver for quad leg
        if not biped:
            # duplicate from leg jnts
            spring_jnts = pm.duplicate(jnts, po=1)
            for i, j in enumerate(spring_jnts):
                if i == 0:
                    j.rename(common.replace_name(jnts[0].name(), 'joint', 'spring:joint'))
                    continue
                common.replace_name(j, 'joint', 'spring:joint')

            # constrains hip spring jnt
            pm.parentConstraint(jnts[0], spring_jnts[0])
            pm.scaleConstraint(jnts[0], spring_jnts[0])

            # create spring ik handle & pole vector
            spring_ikh = pm.ikHandle(name=common.replace_name(spring_jnts[1].name(), 'joint', 'ikHandle'),
                                     sol='ikSpringSolver',
                                     sj=spring_jnts[1], ee=spring_jnts[4])
            spring_ikh[1].rename(spring_ikh[0].name().replace('ikHandle', 'effector'))
            spring_ikh_grp = common.group_pivot(spring_ikh[0], layer=['offset'])
            spring_pv = pm.spaceLocator(name=common.replace_name(spring_ikh[0].name(), 'ikHandle', 'pv:locator'))
            pm.matchTransform(spring_pv, spring_ikh_grp)
            spring_ikh_grp.addChild(spring_pv)
            pm.poleVectorConstraint(spring_pv, spring_ikh[0])

            leg_ik_sys.util_grp.addChild(spring_ikh_grp)
            pm.parentConstraint(tt_con_list[-1], spring_ikh_grp[0], mo=1)

            # create locator to drive ankle and leg ik handle
            leg_spring_loc = common.create_name(side='', name=leg_name[:-1], fn='spring', _type='locator', create=True)
            ball_con_list[0].addChild(leg_spring_loc)
            pm.matchTransform(leg_spring_loc, ball_con_list[0])
            pm.orientConstraint(spring_jnts[3], leg_spring_loc, mo=1)

            leg_spring_aim_blnd = common.create_name(side='', name=leg_name[:-1], fn='spring_aim', _type='blendColors',
                                                     create=True)
            leg_spring_loc.r.connect(leg_spring_aim_blnd.color1)
            leg_spring_aim_blnd.color2.set([0] * 3)
            leg_spring_aim_blnd.output.connect(ball_con_list[1].r)

            # add attribute to toggle aim
            self.ik_con.addAttr('ballAutoAim', dv=1, min=0, max=1, k=1)
            self.ik_con.ballAutoAim.connect(leg_spring_aim_blnd.blender)

        # lock attr
        channels.cb_status([self.ik_con], v=1, lock=1, show=False)
        channels.cb_status([self.pv_con], rotates='xyz', scales='xyz', v=1, lock=1, show=False)
        channels.cb_status([tt_con_list[-2], heel_con_list[-1]], scales='xyz', v=1, lock=1, show=False)
        channels.cb_status([toe_con_list[-1], ball_con_list[-1]], translates='xyz', scales='xyz', v=1, lock=1,
                           show=False)


def rig_leg_ik(jnts, biped=True, mirror=False, scale=1):
    """
    :param jnts: [hip, leg, knee, ankle, foot, toe]
    :param biped: make ik con on ankle
    :param scale: controller size
    :param mirror:
    :return:
    """
    ...
    if not biped:
        ...
        # create spring ik handle & pole vector
        spring_ikh = pm.ikHandle(name=common.replace_name(spring_jnts[1].name(), 'joint', 'ikHandle'), sol='ikSpringSolver',
                                 sj=spring_jnts[1], ee=spring_jnts[4])
        spring_ikh[1].rename(spring_ikh[0].name().replace('ikHandle', 'effector'))
        spring_ikh_grp = common.group_pivot(spring_ikh[0], layer=['offset'])
        spring_pv = pm.spaceLocator(name=common.replace_name(spring_ikh[0].name(), 'ikHandle', 'pv:locator'))
        ...
        # add attribute to toggle aim
        ik_con.addAttr('ballAutoAim', dv=1, min=0, max=1, k=1)
        ik_con.ballAutoAim.connect(leg_spring_aim_blnd.blender)
        pass
